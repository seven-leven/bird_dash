    ...
    if (speciesMatch) {
      if (currentFamily) {
        ...
        currentFamily.species.push({
          ...
          spotted,
          spottedDate: speciesMatch[3] || void 0
        });
      }
      return;
    }
...
var SpeciesCard = React.memo(
  ({ species, onImageClick }) => {
...
import React3, { useCallback, useEffect as useEffect2, useState as useState2 } from "react";
...
var NavigationArrows = ({
  ...
  return /* @__PURE__ */ React2.createElement("div", { className: "navigation-arrows" }, showPrev ? /* @__PURE__ */ React2.createElement(
    "button",
    {
  ...
  ), showNext ? /* @__PURE__ */ React2.createElement(
    "button",
    {
...
var BirdDashboard = () => {
  ...
  const [expandedSpecies, setExpandedSpecies] = useState3(
    INITIAL_EXPANDED_STATE
  );
  const handleCardClick = useCallback2(
    (species, familyName) => {
      const allSpotted = families.flatMap(
        (f) => f.species.filter((s) => s.spotted)
      );
      const currentIndex = allSpotted.findIndex(
        (s) => s.index === species.index
      );
      setExpandedSpecies({
        species,
        family: familyName,
  ...
  const handlePrev = useCallback2(() => handleNavigation("prev"), [
    handleNavigation
  ]);
  const handleNext = useCallback2(() => handleNavigation("next"), [
    handleNavigation
  ]);
  const renderFamilySection = useCallback2(
    (family) => /* @__PURE__ */ React5.createElement(React5.Fragment, { key: family.family }, /* @__PURE__ */ React5.createElement(
      FamilyCard,
      {
        family,
