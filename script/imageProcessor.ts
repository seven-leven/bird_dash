...
export async function verifyIntegrity(
  ...
  return {
    ...
    missingInJson:
      Array.from(rawIds).filter(id => !drawnIds.has(id)),

    missingInRaw:
      Array.from(drawnIds).filter(id => !rawIds.has(id)),

    missingFull:
      Array.from(drawnIds).filter(id => !fullIds.has(id)),

    missingThumb:
      Array.from(drawnIds).filter(id => !thumbIds.has(id)),

    orphanedFull:
      Array.from(fullIds).filter(id => !drawnIds.has(id)),

    orphanedThumb:
      Array.from(thumbIds).filter(id => !drawnIds.has(id)),
  };
}