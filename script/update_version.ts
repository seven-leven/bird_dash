...
export async function updateVersion() {
  ...
  if (jsonCount !== thumbCount || jsonCount !== fullCount) {
    console.error('❌ INTEGRITY CHECK FAILED: Counts do not match!');

    if (jsonCount !== thumbCount) {
      console.error(`   👉 JSON says ${jsonCount}, but found ${thumbCount} thumbnails.`);
    }
    if (jsonCount !== fullCount) {
      console.error(`   👉 JSON says ${jsonCount}, but found ${fullCount} full images.`);
    }
    if (thumbCount !== fullCount) {
      console.error(`   👉 Image folders mismatch (Thumb: ${thumbCount} vs Full: ${fullCount}).`);
    }

    Deno.exit(1); // Stop the ship!
  }

  console.log('✅ Integrity Check Passed. All counts tally.');
