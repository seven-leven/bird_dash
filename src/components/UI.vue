<template>
  <!-- Root: full screen, flex COLUMN so header spans full width above sidebar+main -->
  <div class="flex flex-col h-screen w-full overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">

    <!-- ══════════════════════════════════════════════════════════════════════
         FULL-WIDTH HEADER — sits above BOTH sidebar and main content
         Single row: hamburger | collection tabs | divider | section title | → | search | view toggle | progress | theme
         ══════════════════════════════════════════════════════════════════════ -->
    <header class="flex items-center gap-3 h-14 px-3 shrink-0 z-50
      ...
      <button
        ...
        class="lg:hidden p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-slate-200 transition-colors shrink-0"
      ...
      <!-- Divider -->
      <div class="w-px h-6 bg-slate-700 shrink-0 hidden sm:block" />

      <!-- Current section title (fills remaining space) -->
      <span
        v-if="!loading && !error && activeSection"
        class="text-sm font-semibold text-slate-300 truncate hidden md:block flex-1 min-w-0"
      >
        {{ activeSection }}
      </span>
      ...
      <div class="relative w-36 sm:w-48 md:w-56 shrink-0">
        ...
        <input
          ...
          class="block w-full pl-7 pr-6 py-1.5 rounded-md text-sm
      ...
      <!-- Divider -->
      <div class="w-px h-6 bg-slate-700 shrink-0" />

      <!-- Global progress: total drawn / total across ALL collections -->
      <div
        v-if="!loading"
        class="flex flex-col items-end shrink-0 gap-0.5"
        :title="`${globalStats.drawn} drawn out of ${globalStats.total} total across all collections`"
      >
        <div class="flex items-baseline gap-1">
          <span class="text-sm font-bold text-slate-100 tabular-nums">{{ globalStats.drawn }}</span>
          <span class="text-xs text-slate-500">/</span>
          <span class="text-xs text-slate-400 tabular-nums">{{ globalStats.total }}</span>
        </div>
      ...
      <button
        ...
        class="flex items-center justify-center w-8 h-8 rounded-full transition-colors
               bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-yellow-400 shrink-0"
    ...
    <!-- ══════════════════════════════════════════════════════════════════════
         BODY ROW — sidebar + main content side by side, below the header
         ══════════════════════════════════════════════════════════════════════ -->
    <div class="flex flex-1 overflow-hidden">
      ...
      >
        <!-- Sidebar header — label uses groupLabel from active collection -->
        ...
        <div class="flex-1 overflow-y-auto px-4 pb-6 custom-scrollbar">
          <ul v-if="sidebarItems.length" class="space-y-1">
            ...
            >
              ...
              <span class="text-xs shrink-0" :class="item.disabled ? 'opacity-50' : 'opacity-60'">
                <template v-if="viewMode === 'group' && item.total">
