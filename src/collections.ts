    ...
    links: [
      {
        ...
        url: item => `https://www.google.com/search?q=${encodeURIComponent(item.commonName + ' ebird')}`,
      },
      {
        label: 'BirdLife',
        color: 'bg-rose-600 hover:bg-rose-500',
        url: item => `https://datazone.birdlife.org/search?search=${encodeURIComponent(item.commonName)}`,
      },
      {
        label: 'Wiki',
        color: 'bg-slate-600 hover:bg-slate-500',
        url: item => `https://en.wikipedia.org/wiki/Special:Search?search=${encodeURIComponent(item.commonName)}`,
      },
    ],
  },
  {
    ...
    links: [
      {
        label: 'iNaturalist',
        color: 'bg-emerald-600 hover:bg-emerald-500',
        url: item => `https://www.inaturalist.org/search?q=${encodeURIComponent(item.commonName)}`,
      },
      {
        label: 'FishBase',
        color: 'bg-blue-600 hover:bg-blue-500',
        url: item => `https://fishbase.se/search.php?q=${encodeURIComponent(item.scientificName)}`,
      },
      {
        label: 'Wiki',
        color: 'bg-slate-600 hover:bg-slate-500',
